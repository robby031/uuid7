 ...
 *
 * Di library ini, layout internal PRNG dan counter disesuaikan untuk memudahkan implementasi:
...
static int get_version(const uint8_t *uuid) {
    return uuid[6] >> 4;
}

static int get_variant(const uint8_t *uuid) {
    return uuid[8] >> 6;
}
...
static void test_version_variant_consistency(void) {
    ...
    for (int limit = 0; ctx.last_timestamp_ms == start && limit < 100000; limit++) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        printf("UUID[%d] version=0x%x variant=0x%x OK\n",
               i, get_version(u[i]), get_variant(u[i]));
    }
}
...
static void test_monotonic_edge_cases(void) {
    ...
    if (memcmp(prev, curr, 16) >= 0) violations++;
    memcpy(prev, curr, 16);
    ...
    assert(violations == 0);
    printf("Monotonisitas terjaga dalam edge case (counter overflow + mundur).\n");
}
